...
use okf_toolkit_core::bundle::{Bundle, Entry};
...
fn concept_type(entry: &Entry) -> &str {
    entry
        ...
        .frontmatter
        .parsed()
        .and_then(okf_toolkit_core::document::Frontmatter::concept_type)
        .unwrap_or("Unknown")
}
...
fn node(entry: &Entry, today: Date) -> Value {
    ...
    json!({
        "data": {
            ...
            "label": frontmatter
                .and_then(okf_toolkit_core::document::Frontmatter::title)
                .unwrap_or_else(|| entry.id.as_str()),
            "type": concept_type,
            "description": frontmatter
                .and_then(okf_toolkit_core::document::Frontmatter::description)
                .unwrap_or_default(),
            "resource": frontmatter
                .and_then(okf_toolkit_core::document::Frontmatter::resource)
                .unwrap_or_default(),
            "tags": frontmatter.map(okf_toolkit_core::document::Frontmatter::tags).unwrap_or_default(),
            "status": status,
            "generated": generated,
            "verified": verified,
