...
use okf_toolkit_core::diagnostic::{Diagnostic, RuleKind, Severity, rule};
    ...
    pub fn new(deny: &[String], allow: &[String], strict: bool) -> Result<Self, String> {
        for code in deny.iter().chain(allow) {
            if rule(code).is_none() {
                return Err(format!(
                    "unknown rule `{code}`; run `okft rules` to list the available codes"
                ));
            }
        }
        Ok(Self {
...
mod tests {
    ...
    fn unknown_rule_codes_are_rejected() {
        ...
        assert!(error.contains("okft rules"));
