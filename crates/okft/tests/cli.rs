//! End-to-end tests driving the compiled `okf` binary.
//!
...
fn okf() -> Command {
    Command::cargo_bin("okf").expect("binary builds")
}
...
fn validate_accepts_every_upstream_bundle() {
    for name in ["acme_retail", "crypto_bitcoin", "ga4", "stackoverflow"] {
        okf().arg("validate").arg(upstream(name)).assert().success();
    }
}
...
fn a_broken_link_fails_only_under_strict_lint() {
    ...
    okf().arg("validate").arg(dir.path()).assert().success();
    okf().arg("lint").arg(dir.path()).assert().success();
    okf()
        .arg("lint")
        .arg(dir.path())
        .arg("--strict")
...
fn rules_lists_every_code() {
    ...
    assert_eq!(
        ...
        okft_core::diagnostic::RULES.len()
    );
}
...
fn help_and_version_exit_zero() {
    okf()
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("okf"));
    okf().arg("--version").assert().success();
    okf().args(["validate", "--help"]).assert().success();
}

#[test]
fn an_unknown_command_is_a_usage_error() {
    okf().arg("frobnicate").assert().code(EXIT_USAGE);
}
