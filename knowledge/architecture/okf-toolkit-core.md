...
`okf-toolkit-core` owns everything that understands OKF. It reads a directory of
...
or `rmcp`, so the [MCP server](okf-toolkit-mcp.md) and the
[web viewer](okf-toolkit-web.md) reuse it directly rather than shelling out to
the [CLI](okf-toolkit-cli.md).
